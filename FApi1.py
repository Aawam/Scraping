...
def get_products_items(soup):
    item_products = soup.find_all("div", class_="o_wsale_product_grid_wrapper position-relative h-100")
    return item_products
...
def parsing(item_products):
    ...
    for product in item_products:
        ...
        harga = product.find('span', class_ = 'h6 mb-0').find('span', class_='oe_currency_value').get_text().replace("Rp", "").replace(".", "").replace(" ", "")
        installment = product.find('span', class_='bmd-installment').get_text().replace("Cicilan Rp. ", "").replace("/bln", "").replace(".", "")
        location = product.find('div', class_='o_wsale_product_sub d-flex justify-content-between align-items-end pb-1').find('div', class_='d-flex flex-column').find('span', class_='bmd-installment').get_text().replace("\n", "").replace(" ", "")
        url = product.find('a', class_='text-primary text-decoration-none')['href'].replace(" ", "")
        final_url = f"https://www.bhinneka.com/{url}"
        output = {"Name": nama, "Price": harga, "Installment": installment, "Location": location, "URL": final_url}
        final_output.append(output)
    return final_output
...
def scrape_product_details(product_url):
    ...
    soup = convert_to_bs4_obj(src)
    estimated_arrival_element = soup.find('div', class_ = 'js_product js_main_product mb-3').find('div', class_ = 'row product-data-row').find('div', class_ = 'col-sm-9').find('span', id_ = 'productHandlingTimeInformation').get_text()
    estimated_arrival = estimated_arrival_element.text if estimated_arrival_element else None
